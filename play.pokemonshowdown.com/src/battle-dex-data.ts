...
type TypeName = 'Normal' | 'Fighting' | 'Flying' | 'Poison' | 'Ground' | 'Rock' | 'Bug' | 'Ghost' | 'Steel' |
	'Fire' | 'Water' | 'Grass' | 'Electric' | 'Psychic' | 'Ice' | 'Dragon' | 'Dark' | 'Fairy' | 'Stellar' | 'Crystal' |'???';
...
class Species implements Effect {
	...
	constructor(id: ID, name: string, data: any) {
		...
		this.isTotem = false;
		this.isMega = !!(this.forme && ['-mega', '-megax', '-megay', '-megam', '-megaf', '-crystal'].includes(this.formeid));
