...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | Dex.PokemonSet = '' as ID) {
		...
		if (formatName in window.Formats) {
			...
			if ('mod' in info) {
				...
				if (info.mod.includes('infinitefusion') || info.mod.includes('pokeathlon') || info.mod.includes('insurgence') || info.mod.includes('uranium') || info.mod.includes('infinity') || info.mod.includes('mariomon') || info.mod.includes('chaos')) {
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		if (['gen9infinitefusion', 'gen7infinitefusion', 'gen9pokeathlon', 'gen9pokeathlon2', 'gen9insurgence', 'gen6insurgence', 'gen9uranium', 'gen6uranium', 'gen9infinity', 'gen6infinity', 'gen9mariomon', 'gen9chaos', 'gen9chaosfusion'].includes(this.dex.modid)) table = table[this.dex.modid];
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		if (['gen9infinitefusion', 'gen7infinitefusion', 'gen9pokeathlon', 'gen9pokeathlon2', 'gen9insurgence', 'gen6insurgence', 'gen9uranium', 'gen6uranium', 'gen9infinity', 'gen6infinity', 'gen9mariomon', 'gen9chaos', 'gen9chaosfusion'].includes(this.dex.modid)) lsetTable = lsetTable[this.dex.modid];
		while (learnsetid) {
...
const fusionMoves: {[key: string]: {[key: string]: string[]}[]} = {
	...
	"clangingscales": [{"fusion": ["ekans", "arbok", "garchomp", "flygon", "haxorus"]}],
	...
	"flyingpress": [{"fusion": ["torchic", "combusken", "blaziken", "farfetchd", "heracross"]}, {"type": ["Flying", "Fighting"]}],
	...
	"needlearm": [{"fusion": ["ferrothorn"]}],
	"forestscurse": [{"type": ["Grass", "Ghost"]}],
	"spikyshield": [{"fusion": ["ferroseed", "ferrothorn"]}, {"fusion": ["sandslash", "jolteon", "cloyster"], "type": ["Grass"]}],
	...
	"shoreup": [{"fusion": ["muk", "grimer"], "type": ["Ground"]}],
	"icehammer": [{"type": ["Ice"], "learns": ["crabhammer", "woodhammer"]}],
	"multiattack": [{"fusion": ["arceus", "mew", "genesect"]}],
	"revelationdance": [{"fusion": ["kecleon", "bellossom", "cleffa", "clefairy", "clefable"]}],
	"banefulbunker": [{"fusion": ["tentacool", "tentacruel", "nidorina", "nidorino", "nidoqueen", "nidoking", "grimer", "muk", "qwilfish"]}],
	...
	"relicsong": [{"fusion": ["jynx", "lapras", "jigglypuff", "wigglytuff", "misdreavus", "mismagius"]}],
