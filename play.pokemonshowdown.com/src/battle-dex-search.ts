	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | PokemonSet = '' as ID) {
		...
		if (formatName in window.Formats) {
			...
			if ('mod' in info) {
				...
				if (info.mod.includes('infinitefusion') || info.mod.includes('pokeathlon') || info.mod.includes('insurgence')) {
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		if (['gen7infinitefusion', 'gen9pokeathlon', 'gen9insurgence', 'gen6insurgence'].includes(this.dex.modid)) lsetTable = lsetTable[this.dex.modid];
		while (learnsetid) {
