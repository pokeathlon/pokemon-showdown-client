...
export class Battle {
	...
	changeWeather(weatherName: string, poke?: Pokemon, isUpkeep?: boolean, ability?: Effect) {
		...
		if (weather) {
			...
			if (poke) {
				...
				this.weatherTimeLeft = (this.gen <= 5 || isExtremeWeather || (window.Formats[toID(this.tier)].mod === 'gen7infinitefusion' && !this.tier.includes('Draft'))) ? 0 : 8;
				this.weatherMinTimeLeft = (this.gen <= 5 || isExtremeWeather || (window.Formats[toID(this.tier)].mod === 'gen7infinitefusion' && !this.tier.includes('Draft'))) ? 0 : 5;
	...
	runMinor(args: Args, kwArgs: KWArgs, nextArgs?: Args, nextKwargs?: KWArgs) {
		...
		case '-transform': {
			...
			poke.addVolatile('transform' as ID, pokemon, shiny, gender, level, tpoke.fusion, tpoke.altsprite);
