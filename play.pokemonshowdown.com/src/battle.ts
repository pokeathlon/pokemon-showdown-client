...
export class Battle {
	...
	changeWeather(weatherName: string, poke?: Pokemon, isUpkeep?: boolean, ability?: Effect) {
		...
		if (weather) {
			...
			if (poke) {
				...
				this.weatherTimeLeft = (this.gen <= 5 || isExtremeWeather || (this.tier.includes('IF Dex') && !this.tier.includes('Draft'))) ? 0 : 8;
				this.weatherMinTimeLeft = (this.gen <= 5 || isExtremeWeather || (this.tier.includes('IF Dex') && !this.tier.includes('Draft'))) ? 0 : 5;
	...
	runMinor(args: Args, kwArgs: KWArgs, nextArgs?: Args, nextKwargs?: KWArgs) {
		...
		case '-status': {
			...
			if (poke) poke.status = args[2] as StatusName;
			this.activateAbility(ofpoke || poke, effect);
			if (effect.effectType === 'Item') {
