...
export class BattleTextParser {
	...
	static upgradeArgs({ args, kwArgs }: { args: Args, kwArgs: KWArgs }): { args: Args, kwArgs: KWArgs } {
		...
		case '-activate': {
			...
			if ([
				'bind', 'greatbind', 'wrap', 'clamp', 'whirlpool', 'firespin', 'magmastorm', 'sandtomb', 'infestation', 'snaptrap', 'thundercage', 'trapped', 'thornprison', 'sandsnare',
			].includes(id)) {
	...
	parseArgsInner(args: Args, kwArgs: KWArgs) {
		...
		case '-end': {
			...
			const line1 = this.maybeAbility(effect, pokemon) || this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
			let id = BattleTextParser.effectId(effect);
			if (id === 'doomdesire' || id === 'futuresight' || id === 'spudmortar' || id === 'chitinsnare' || id === 'flockshock') {
		...
		case '-fail': {
			...
			if (['desolateland', 'primordialsea'].includes(blocker) &&
				!['sunnyday', 'raindance', 'sandstorm', 'newmoon', 'thunderstorm', 'fallout', 'hail', 'snowscape', 'chillyreception'].includes(id)) {
			...
			templateId = 'fail';
			if (['brn', 'frz', 'frb', 'par', 'psn', 'slp', 'substitute', 'shedtail'].includes(id)) {
