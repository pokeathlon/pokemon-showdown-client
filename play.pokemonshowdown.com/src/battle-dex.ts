...
export const Dex = new class implements ModdedDex {
	...
	getSpriteData(pokemon: Pokemon | Species | string, isFront: boolean, options: {
		...
		for (var fangame of ["Infinite Fusion", "Pokeathlon", "Insurgence", "Uranium", "Infinity", "Mariomon", "Rejuvenation"]) {
	...
	getPokemonIcon(pokemon: string | Pokemon | ServerPokemon | Dex.PokemonSet | null, facingLeft?: boolean) {
		...
		let left = (num % 12) * 40;
		return `image-rendering:pixelated;${fusion.length ? 'animation: rainbowshadow ' + ((Math.random() * 2) + 2) + 's infinite;' : ''}background:transparent url(${Dex.resourcePrefix}sprites/pokemonicons-sheet.png?v19) no-repeat scroll -${left}px -${top}px${fainted}`;
	}
	...
	getItemIcon(item: any) {
		...
		if (num === -6) return 'background:transparent url(https://play.pokeathlon.com/sprites/fangame-sprites/rejuvenation/items/' + item.id + '.png) center/24px';
